...
class GetMensagensPorHorarioUseCase:
    def __init__(self, target_resolver_service: TargetResolverService, mensagens_service: MensagemService):
    ...
    def execute(self, filtros: MensagensRequestDTO) -> List[Dict]:
        numero_ids, tickets = self.target_resolver_service.resolver_alvos(filtros)

        return self.mensagens_service.obter_quantidade_mensagens_por_horario(
            numeros=numero_ids,
            tickets=tickets,
            tipo=filtros.tipo,
            grupo=filtros.grupo,
